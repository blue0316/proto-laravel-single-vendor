    ...
    <div class="card card-modern">
        ...
        <div class="card-body">
            <div class="table-responsive">
                ...
                <table
                    class="table table-ecommerce-simple table-ecommerce-simple-border-bottom table-striped table-no-collapse mb-2"
                    v-if="shippingMethod && order.order_type != 'suborder'"
                >
                    <tbody>
                        <tr>
                            ...
                            <td width="50%">{{ shippingMethod }}</td>
                            <td colspan="2"></td>
                            <td width="10%" class="text-right">
                                <span v-html="formatPrice(order.shipping_cost)"></span>
                                <div
                                    class="text-color-danger text-1"
                                    v-if="refundedShippingCost < 0"
                                >
                                    <i class="fa fa-reply mr-2"></i>
                                    <span
                                        v-html="
											formatPrice(refundedShippingCost)
										"
                                    ></span>
                                </div>
                                <input
                                    ...
                                    v-show="toggleRefund"
                                    type="text"
                                    v-model.number="refundShippingCost"
                                />
                            </td>
                            <td width="10%" class="text-right" v-if="enableTax">
                                <span v-html="formatPrice(order.shipping_tax)"></span>
                                <div
                                    class="text-color-danger text-1"
                                    v-if="refundedShippingTax < 0"
                                >
                                    <i class="fa fa-reply mr-2"></i>
                                    <span
                                        v-html="
											formatPrice(refundedShippingTax)
										"
                                    ></span>
                                </div>
                                <input
                                    ...
                                    v-show="toggleRefund"
                                    type="text"
                                    v-model.number="refundShippingTax"
                                />
                            </td>
                            <td class="actions" width="100"></td>
                        </tr>
            ...
            <div class="row justify-content-end flex-column flex-lg-row my-3">
                ...
                >
                    ...
                    <span class="d-flex align-items-center">
                        <span v-if="this.order.order_type != 'suborder'">{{ order.coupons.length }}</span>
                        coupons
                        <i
...
export default {
    ...
    computed: {
        ...
        totalTax: function () {
            if (this.order.order_type == "suborder") {
                ...
                return (
                    this.order.items.reduce((acc, cur) => {
                        return (
                            acc +
                            cur.tax_amount *
                                (1 + cur.coupon_amount / cur.net_revenue)
                        );
                    }, 0) +
                    this.order.shipping_tax * 1.0 +
                    this.couponTax
                );
            }
        },
    ...
    created: function () {
        this.refunded = [...this.alreadyRefunded];
