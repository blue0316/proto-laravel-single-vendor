<template>
	<div class="card card-modern">
		...
		<div class="card-body">
			...
			<div class="row">
				<div class="col-lg-4">
					<ReportCustomerDoughnutComponent :sales="doughnut" />
				</div>
				<div class="col-lg-8 mt-3 mt-lg-0">
					<ReportCustomerBarComponent :reports="reports" :date="date" />
				</div>
			</div>
		</div>
	</div>
...
import ReportCustomerBarComponent from "./ReportCustomerBarComponent.vue";
import ReportCustomerDoughnutComponent from "./ReportCustomerDoughnutComponent.vue";

export default {
	components: {
		DatePicker,
		ReportCustomerBarComponent,
		ReportCustomerDoughnutComponent,
	},
	props: {},
	data() {
		return {
	...
	methods: {
		getData: async function () {
			...
			await window.axios
				.get("/admin/ecommerce/reports/get-customers", {
					params: {
						start: this.date[0],
						end: this.date[1],
				...
				.then((response) => {
					...
					this.doughnut = [
						response.data.customer_sales,
						response.data.guest_sales,
					];
				})
				.catch((error) => {
					this.$vToastify.stopLoader();
