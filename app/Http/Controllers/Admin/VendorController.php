...
class VendorController extends Controller {
	...
	public function getSetting( $id ) {
		...
		if ( $user->role_id != 4 || ( $user->role_id == 4 && $user->id == $id ) ) {
			...
			$vendor = User::with( array( 'vendor', 'vendor.banner', 'vendor.profile' ) )
							->findOrFail( $id );
			return view(
				'admin.ecommerce.vendors.setting',
				array(
					'media'  => $media,
					'vendor' => $vendor,
				)
	...
	public function getWithdraw( Request $request ) {
		...
		return view(
			'admin.ecommerce.vendors.withdraw',
			array(
				'withdraws' => $withdraws,
				'can_add'   => $can_add,
	...
	public function getVendors( Request $request ) {
		...
		$vendors     = User::with( array( 'vendor' ) )
						->where(
						...
						->where( 'role_id', 4 )
						->paginate( 20 );
		return view( 'admin.multivendor.vendors', array( 'vendors' => $vendors ) );
	}
	...
	public function getAllWithdraws( Request $request ) {
		$request->flash();
		$filter_by = $request->input( 'filter-by', '*' );
		$withdraws = UserWithdraw::with( array( 'user.vendor' ) );
		...
		return view( 'admin.multivendor.withdraw', array( 'withdraws' => $withdraws ) );
	}
}
