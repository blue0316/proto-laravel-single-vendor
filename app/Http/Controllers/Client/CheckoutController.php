	...
	public function placeOrder( Request $request ) {
		...
		if ( $payment_method['slug'] === 'cash_on_delivery' ) {
			...
			$secret_key  = PaymentMethodDetail::where( 'payment_method_id', $payment_method['id'] )
											->where( 'meta', $environment . '_secret_key' )
											->first()
											->value;
			//  require file
	...
	public function stripeHookHandler( Request $request ) {
		$order_info = json_encode(
			array(
				...
				'payment_intent'       => $request->input( 'data' )['object']['payment_intent'],
				'session_amount_total' => $request->input( 'data' )['object']['amount_total'],
			)
		);
		...
		if ( $request->input( 'type' ) == 'checkout.session.completed' ) {
			...
			Mail::to( $order->customer_email )->send( new PaymentSuccess( $order, $user->email ) );
		} else {
			$new_status = 'failed';
		}
