...
class VendorController extends Controller {
	...
	public function getVendors( Request $request ) {
		$vendors = User::with( array( 'vendor:user_id,banner_image,profile_image,country,state,store_name', 'vendor.banner:id,copy_link,width,height', 'vendor.profile:id,copy_link,width,height' ) )
						->where( 'role_id', 4 )
						->select( array( 'created_at', 'id', 'role_id' ) )
						->whereHas(
							'vendor',
							function ( $query ) {
		...
		return response( array( 'vendors' => $vendors ) );
	}
	...
	public function getSingleVendor( Request $request, $id ) {
		$per_page = 9;
		$vendor   = User::with( array( 'vendor:user_id,banner_image,profile_image,country,state,store_name', 'vendor.banner:id,copy_link,width,height', 'vendor.profile:id,copy_link,width,height' ) )
						->where( 'role_id', 4 )
						->select( array( 'created_at', 'id', 'role_id' ) )
						->whereHas(
							'vendor',
		...
		$products = Product::select( array( 'id', 'name', 'slug', 'parent', 'type', 'price', 'sale_end', 'sale_price', 'sale_schedule', 'sale_start', 'excerpt', 'short_desc', 'featured', 'manage_stock', 'stock_status', 'stock_quantity', 'sold_individually' ) )
							->with( array( 'media:copy_link', 'categories' ) )
							->where( 'author_id', $id )
							->where( 'parent', 0 );
		if ( $request->filled( 'category' ) ) {
			...
			if ( $certain_category->parent == -1 ) {
				$parent_categories = collect( array() )->prepend( $certain_category )->all();
			} else {
