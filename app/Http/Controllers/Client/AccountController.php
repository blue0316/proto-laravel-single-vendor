...
class AccountController extends Controller {
	...
	public function getCustomerOrders( $customer ) {
		...
		$orders = Order::with( 'items' )->select( array( 'id', 'status', 'order_total_price', 'order_refunded_price', 'order_total_qty', 'created_at' ) )
	...
	public function getOrderDetail( $id, Request $request ) {
		...
		$refunded_items = OrderItem::whereHas(
			'order',
	...
	public function getDownloadableProducts( $customer ) {
		$items_group_by_order = OrderItem::with(
			array(
				'product:id,slug',
				'product.files',
				'order' => function ( $query ) {
							...
							->whereHas(
								...
								function ( $query ) use ( $customer ) {
									$query->where( 'customer_email', $customer )->whereIn( 'status', array( 'completed', 'processing', 'on-hold' ) )->where( 'parent', 0 );
								}
							)
							->has( 'product.files' )
							->latest()
	...
	public function changeAccountDetail( Request $request ) {
		...
		if ( $data['email'] && $user->email !== $data['email'] ) {
			Order::where( 'customer_email', $user->email )->update( array( 'customer_email' => $data['email'] ) );
	...
	public function changeAccountBillingAddress( Request $request ) {
		...
		$user = User::findOrFail( $data['id'] );
		$user->fill( Arr::except( $data, array( 'id' ) ) );
		$user->save();
		return response( $user, 202 );
	}
