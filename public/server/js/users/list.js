$( document ).ready( function () {
    ...
    var usersArray; // Users to remove    
    ...
    */
    $( '.select-all' ).on( 'change', function ( ev ) {
        ...
        if ( $( this ).is( ':checked' ) ) {
            $( ".table tbody .checkbox-style-1:not(:disabled)" ).prop( "checked", true );
        } else {
    ...
    $( ".bulk-action-apply" ).click( function ( e ) {
        ...
        for ( var $i = 0; $i < $checks.length; $i++ ) {
            usersArray.push( $checks.eq( $i ).data( 'id' ) );
        }

        if ( usersArray.length == 0 || $( ".bulk-action" ).val() != "delete" ) {
            new PNotify( {
                title: 'Warning',
